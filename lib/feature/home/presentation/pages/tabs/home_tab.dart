...
class HomeTab extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return CustomScrollView(
      slivers: [
        SliverToBoxAdapter(
          child: _titleArea(context, hasAi: aiStatusAsync.value ?? false),
        ),
        SliverToBoxAdapter(
          child: aiStatusAsync.when(
            data: (isOnline) =>
                ActivationCard.ai(isActivated: isOnline, title: 'AI'),
            error: (e, s) => ActivationCard.ai(isActivated: false, title: 'AI'),
            loading: () => const Loading(),
          ),
        ),
        SliverToBoxAdapter(child: SizedBox(height: 8.h)),
        SliverToBoxAdapter(
          child: isRootAsync.when(
            data: (isRoot) {
                    ...
                    data: (isZygiskModuleInstalled) {
                      if (isZygiskModuleInstalled) {
                        ...
                        return TextButton(
                          ...
                          child: Text(context.isChinese?"下载Magisk模块":"Download module for Magisk"),
                        );
                      }
                    },
